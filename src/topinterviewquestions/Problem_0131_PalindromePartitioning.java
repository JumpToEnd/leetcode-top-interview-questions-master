...
public class Problem_0131_PalindromePartitioning {
	...
	/**
	 * 对角线位置，0..0位置，1..1位置，2..2位置,.....  肯定都是true
	 * 第二条对角线位置，0..1位置，1..2位置，2..3位置,....  对比原字符串种两个字符是否相等
	 *
	 * 普遍位置依赖分析：
	 * dp[i][j] 前提 str[i] == str[j]，同时 dp[i-1][j-1] 为true
	 */
	...
	public static void process(String s, int index, LinkedList<String> path, boolean[][] dp, List<List<String>> ans) {
		if (index == s.length()) {
			...
			// 前提：从0...index位置已经是回文了
			// 从index往后挨个试，如果从index...end是回文，尝试去分解剩余的位置
